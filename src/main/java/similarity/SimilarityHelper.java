...
public class SimilarityHelper {
    ...
    public void mostSimilarSourceTFIDF() throws IOException {
    ...
    public void mostSimilarSourceWord2Vec() throws IOException, NoSuchMethodException, IllegalAccessException, InvocationTargetException {
        ...
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(similarity.getFile().substring(0, similarity.getFile().length()-4) + "_word2vec_" + similarity.getSource() + ".csv", true))) {
            ...
            w.getSentences(similarity.getBenchmarkDataset(), benchmarkSentences);

            List<Collection<String>> bugSentences = new ArrayList<>();
            w.getSentences(similarity.getFile(), bugSentences);
